...
#### 非凸惩罚函数回归
- 算子说明
弹性网族惩罚函数均为凸函数，且是有偏估计，通过减小预测模型的方差达到预测总误差的目的。为了进一步降低预测总误差,Fan和Li提出SCAD回归，它不仅对于很小的回归系数压缩到0，而且是一种近似的无偏估计。
2010年，zhang提出MCP回归同样是可微的连续非凸惩罚函数，保留了SCAD的渐进无偏估计优点，对回归系数采取了有差别的惩罚，得到更精确的估计。
